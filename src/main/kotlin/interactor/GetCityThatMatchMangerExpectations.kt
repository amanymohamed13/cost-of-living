...
class GetCityThatMatchMangerExpectations(private val dataSource: CostOfLivingDataSource) {
    fun execute(boundary: Float = Float.MAX_VALUE): CityEntity? {
        ...
        val wantedPrice =
            (cityDataHolders.maxOf { it.priciestMeal } + cityDataHolders.minOf { it.cheapestMeal }) / 2
        val mostSuitableDataHolder =
            cityDataHolders.minByOrNull { abs(it.minBoundary(wantedPrice)) }
    ...
    class CityMealsDataHolder(val city: CityEntity, vararg mealsPrices: Float?) {
        ...
        fun minBoundary(price: Float) = filteredMeals.fold(Float.MAX_VALUE) { acc, fl ->
            val x = abs(fl - price)
            if (x < acc) x else acc
        }
